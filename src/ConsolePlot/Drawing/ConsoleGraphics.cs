            ...
            bool ensureVisible = false)
        {
            if (ensureVisible)
            {
                if (direction == TextDirection.Horizontal)
                {
                    ...
                    x = Math.Min(Math.Max(x, ClipBounds.Left), ClipBounds.Right - s.Length + 1);
                    y = Math.Min(Math.Max(y, ClipBounds.Bottom), ClipBounds.Top);
                }
                else
                {
                    ...
                    x = Math.Min(Math.Max(x, ClipBounds.Left), ClipBounds.Right);
                    y = Math.Min(Math.Max(y, ClipBounds.Bottom + s.Length - 1), ClipBounds.Top);
                }
            }
